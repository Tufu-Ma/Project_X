...
// Config สำหรับ multer
const upload = multer({ dest: 'uploads/' });

// Route สำหรับเพิ่มสินค้า
router.post('/', upload.single('image'), (req, res) => {
    console.log('Received product data:', req.body);  // log ข้อมูลที่ได้รับจาก frontend
    console.log('Uploaded file:', req.file);  // log ข้อมูลไฟล์ที่อัปโหลด
    ...
    // สร้าง query สำหรับเพิ่มข้อมูลสินค้า
    const query = `
...
router.get('/', (req, res) => {
    ...
    requestSql.query(query, (err, result) => {
        ...
        res.send(result.recordset);  // ส่งข้อมูลสินค้าทั้งหมดกลับไปยัง client
    });
});
...
router.put('/:id', upload.single('image'), (req, res) => {
    ...
    // สร้าง query สำหรับแก้ไขข้อมูลสินค้า
    const query = `
        UPDATE Products
